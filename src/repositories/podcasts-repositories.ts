...
export const repositoryGetPodcastDetails = async (podcastId: string): Promise<PodcastModel | null> => {
  const language = "utf-8";
  const rawData = await fs.readFile(pathData, language);
  const podcasts: PodcastModel[] = JSON.parse(rawData);
  
  // Encontrar o podcast pelo ID (assumindo que o ID é o videoId)
  const podcast = podcasts.find((p: PodcastModel) => p.videoId === podcastId);
